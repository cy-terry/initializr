 ...
 */
public class WiiWebMavenBuildCustomizer extends WiiAppDependencyBuild<MavenBuild> implements BuildCustomizer<MavenBuild> {
    ...
    public WiiWebMavenBuildCustomizer(ProjectDescription description, InitializrMetadata metadata, BuildItemResolver buildItemResolver) {
    ...
    public void customize(MavenBuild build) {
        MavenBuild webModel = new MavenBuild(buildItemResolver);

        project(webModel);
        parent(webModel);
        dependency(webModel);

        build.addModel(webModel);
    }
    ...
    private void project(MavenBuild build) {
        build.settings().artifact(WiiModelUtil.webName(description))
